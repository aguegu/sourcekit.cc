...
- PWM 速度控制 (~220Hz)，支持前进/后退/刹车功能

**技术说明**：RX4M4S 生成两种类型的 PWM 信号：
- **电机 PWM**：~220Hz 频率，实现平滑的电机速度控制。每个直流电机通道使用两个互补的 PWM 输出来驱动 H桥，实现前进/后退/刹车操作。
...
| 显示内容 | 调试值：前 8 个通道值（通道 0-7）、4 个电机输出和 4 个舵机输出，以有符号 3 位整数显示在 4 行 OLED 上 |

**显示说明**：MeshMass 显示原始十进制数值（非滚动条或进度指示器），便于精确验证。4 行 OLED 以有符号 3 位整数显示调试信息：第 1-2 行显示前 8 个通道值（通道 0-7，每行 4 个值），第 3 行显示 4 个电机输出值，第 4 行显示 4 个舵机输出值。这使学生能够调试从通道到输出的映射和数学运算。OLED 屏幕单独出售，对于固定安装是可选的，对预算有限的构建友好。
...
RX4M4S 可通过 [MeshMass 在线平台](https://meshmass.com) 进行编程。用户可以：

- 将任何 16 个无线通道映射到电机和舵机输出
...
固件从配对的 TX6A4D 发射器接收 16 个有符号字节（-128 至 127）作为无线通道。这些通道构成发射器和接收器之间的通信桥梁：
- 通道 0-5：通常从模拟输入（摇杆、旋钮）映射
- 通道 6-15：通常从数字输入（按钮、逻辑）映射
...
- `int8_t getChannel(uint8_t n)`：从无线通道 `n` (0-15) 返回接收到的有符号 8 位值 (-128 到 127)
- `void setMotor(uint8_t n, int8_t value)`：将电机 `n` (0-3) 设置为 `value` (-127 到 127)。正值 = 前进，负值 = 后退，0 = 停止
- `void setServo(uint8_t n, uint8_t value)`：设置舵机 `n` (0-3) 的正脉冲宽度。该值表示脉冲宽度，单位为 0.01ms（例如，150 = 1.5ms），具有固定的 20ms 周期。参数为 `uint8_t` (0-255 范围)，支持约 0.5-2.5ms 脉冲宽度。典型中心为 ~150 (1.5ms)，范围 100-200 (1.0-2.0ms)。使用公式 `150 + 通道 * 2 / 5` 将通道值 (-127 到 127) 转换为舵机范围，或调整缩放因子 (0.4 = *2/5) 以适应不同的舵机行程范围。
- `void loop()`：应用程序主循环函数，在每次成功接收无线数据包后调用（连接正常时通常为 50Hz）。实现此函数以定义输出控制逻辑。
...
框架自动处理多个系统功能：
- **OLED 显示**：实时调试显示，显示调试值：前 8 个通道值（通道 0-7）、4 个电机输出和 4 个舵机输出，以有符号 3 位整数显示在 4 行 OLED 上。显示原始十进制数值（非简化可视化），便于精确验证。OLED 单独出售，对于固定安装是可选的。
- **WS2812 Neopixels**：可寻址 RGB LED 灯带，用于车辆灯光（头灯/转向灯/尾灯）。**与 SM3 共享信号引脚**（需要特殊固件）
- **音频模块**：MP3 播放音效（引擎启动、喇叭）。**与 SM1、SM2 共享引脚**（需要特殊固件，模块单独出售）
- **无线通信**：管理 2.4GHz 数据包接收与自动跳频
- **电池监测**：监测输入电压并提供低电量警告

**固件变体：**
- **RX4M4S**：4 路直流电机 + 4 路舵机（SM0-SM3），WS2812 Neopixels 和音频模块接口禁用（当前可用）
- **RX4M3S1N**：4 路直流电机 + WS2812 Neopixels（车辆灯光，使用 SM3 引脚）+ 3 路舵机（SM0-SM2）- *尚未发布*
- **RX4M1S1N1A**：4 路直流电机 + WS2812 Neopixels（车辆灯光）+ 音频模块（MP3 音效，使用 SM1、SM2 引脚）+ 1 路舵机（仅 SM0）- *尚未发布，音频模块正在开发中*
