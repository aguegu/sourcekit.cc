...
- **Confirm the right firmware is on each board** (the kit ships pre-flashed; re-flash from the MeshMass platform if needed):
  - **RX4M4S** — *RX4MX* course, lesson [`02-MiniTank`](https://meshmass.com/#/courses/12136090/lessons/02-MiniTank). This is the program that actually drives the tank: it turns stick positions into left and right track speeds.
  - **TX6A4D** — *TX6AX* course, lesson [`00-Default`](https://meshmass.com/#/courses/12132971/lessons/00-Default). The controller has only this one program; it simply forwards every stick and button reading.
