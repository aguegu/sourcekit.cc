...
- **Confirm the right firmware is on each board** (the kit ships pre-flashed; re-flash from the [MeshMass platform](https://meshmass.com) if needed). Open the course, pick the lesson, build and flash:
  - **RX4M4S** — course **RX4MX**, lesson **`02-MiniTank`** ([source](https://github.com/aguegu/chrc-courses/tree/main/rx4mx/lessons/02-MiniTank)). This is the program that actually drives the tank: it turns stick positions into left and right track speeds.
  - **TX6A4D** — course **TX6AX**, lesson **`00-Default`** ([source](https://github.com/aguegu/chrc-courses/tree/main/tx6ax/lessons/00-Default)). The controller has only this one program; it simply forwards every stick and button reading.
