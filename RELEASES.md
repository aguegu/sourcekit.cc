...
- **Lesson links point at the platform.** All 11 RX4M4S lesson links, and the Mini Tank *Pre-class Preparation* links, now use real course keys (`RX4MX` = `12136090`, `TX6AX` = `12132971`) verified against `meshmass.y77.cc/api/courses`. Mini Tank's pre-flash lessons changed meaning with the architecture: the receiver now needs `02-MiniTank` (it holds the driving logic), and the transmitter takes `00-Default`.
